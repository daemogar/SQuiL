...
// Task 7 (Phase 3B): editor parity for the PostgreSQL header model. PostgreSQL is a
// near-twin of SQLite in the generator (both implement ITempTableHeaderDialect) — when the
// resolved dialect is 'postgres', `parseSQuiL` recognizes `Create Temp Table <Prefix>_<Name> (...)`
// declarations (direction/cardinality from the bare name, single-column singular collapsing to a
// scalar) instead of the T-SQL `Declare @...` / `Use` form, exactly like the SQLite dialect.
// Mirrors sqliteHeader.test.ts one-for-one with dialect 'postgres' and PG type spellings.
...
test('no "Missing USE" warning for the Postgres dialect', () => {
  const r = parseSQuiL([
    'Create Temp Table Return_Total (Total integer);',
    'Select 1;',
  ].join('\n'), 'postgres');
  ...
  assert.deepStrictEqual(useWarnings, [], 'PostgreSQL has no USE statement — must not warn');
});

test('a full Postgres query recognizes both declarations across body/sample-DML lines', () => {
  const r = parseSQuiL([
...
// Bracket-quoted table names must be parsed like SQLite (full parity with the generator's
// IdentifierRegex, which strips brackets on both declaration name and DML targets).
test('bracket-quoted table name is parsed into a variable with brackets stripped', () => {
  const r = parseSQuiL([
