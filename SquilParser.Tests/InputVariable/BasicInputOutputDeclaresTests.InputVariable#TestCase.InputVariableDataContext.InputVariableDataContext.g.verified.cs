//HintName: TestCase.InputVariableDataContext.InputVariableDataContext.g.cs
// <auto-generated />
...
partial class InputVariableDataContext : SQuiLBaseDataContext
{
	public async Task<InputVariableResponse> ProcessInputVariableAsync(
		InputVariableRequest request,
		CancellationToken cancellationToken = default!)
	{
		...
		InputVariableResponse response = new();
		var isObject = false;
		var isTable = false;
		var isScaler = false;
			...
			if(tableTag.StartsWith("__SQuiL__Table__Type__"))
			{
				switch (tableTag)
				{
					...
					case "__SQuiL__Table__Type__Returns_Table__":
					{
						...
						do
						{
							if (reader.GetString(0) == "Return_Table")
							{
		...
		string inputParamObject(List<SqlParameter> parameters)
		{
			...
			query.AppendLine("Insert Into @Param_Object(ObjectID, IsMale, FirstName)");
			query.Append("Values (");
			...
			query.Append(", ");
			AddParams(query, parameters, 0, "ParamObject", "FirstName", System.Data.SqlDbType.Bit, request.Object.FirstName, 100);
			query.Append(");");
			query.AppendLine();
		...
		string inputParamsTable(List<SqlParameter> parameters)
		{
			if (request.Table.Count == 0) return "";
			
			System.Text.StringBuilder query = new();
			query.Append("Insert Into @Params_Table(TableID, IsFemale, LastName) Values");
			var comma = "";
			var index = 0;
			foreach(var item in request.Table)
			{
				...
				query.Append(", ");
				AddParams(query, parameters, index, "ParamsTable", "LastName", System.Data.SqlDbType.Bit, item.LastName, 100);
				query.Append(')');
